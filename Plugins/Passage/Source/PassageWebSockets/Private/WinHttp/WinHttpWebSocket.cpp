...
void FWinHttpWebSocket::Connect()
{
		...
		State == EWebSocketConnectionState::Connected)
	{
		...
		UE_LOG(LogPassageWebSockets, Verbose, TEXT("WinHttp WebSocket[%p]: Attempted to connect while in the %s state."), this, LexToString(State));
		return;
	}
	...
	if (!bDisableDomainWhitelist)
	{
		...
		{
			UE_LOG(LogPassageWebSockets, Warning, TEXT("WinHttp WebSocket[%p]: %s is not whitelisted, refusing to connect."), this, *Url);
	...
	{
		UE_LOG(LogPassageWebSockets, Log, TEXT("WinHttp WebSocket[%p]: Domain whitelisting has been disabled by config."), this);
	}

	FWinHttpHttpManager* Manager = FWinHttpHttpManager::GetManager();
	if (!Manager)
	{
		UE_LOG(LogPassageWebSockets, Warning, TEXT("WinHttp WebSocket[%p]: WinHttp Manager shutdown"), this);
...
void FWinHttpWebSocket::Close(const int32 Code, const FString& Reason)
{
	switch (State)
	{
		...
		case EWebSocketConnectionState::Closed:
		{
			...
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("WinHttp WebSocket[%p]: Closed socket while in %s state, ignoring"), this, LexToString(State));
			return;
		}
		case EWebSocketConnectionState::Connecting:
		case EWebSocketConnectionState::Connected:
		{
			...
			{
				// Already closing
				UE_LOG(LogPassageWebSockets, Verbose, TEXT("WinHttp WebSocket[%p]: Call to close while another close was in progress."), this);
				return;
			}
...
void FWinHttpWebSocket::Send(const FString& Data)
{
	if (!IsConnected())
	{
		UE_LOG(LogPassageWebSockets, Warning, TEXT("WinHttp WebSocket[%p]: Failed to send message, we are not connected"), this);
		return;
	}
...
void FWinHttpWebSocket::HandleSessionCreated(FWinHttpSession* SessionPtr)
{
	...
	if (bCloseRequested)
	{
		...
		UE_LOG(LogPassageWebSockets, Warning, TEXT("WinHttp WebSocket[%p]: connection closed before it could start."), this);
	...
	if (!SessionPtr)
	{
		...
		UE_LOG(LogPassageWebSockets, Warning, TEXT("WinHttp WebSocket[%p]: Unable to create WinHttp Session, failing request"), this);
	...
	{
		UE_LOG(LogPassageWebSockets, Warning, TEXT("WinHttp WebSocket[%p]: Failed to create connection"), this);
	...
	{
		UE_LOG(LogPassageWebSockets, Warning, TEXT("WinHttp WebSocket[%p]: Unable to start Connection"), this);
		HandleCloseComplete(EWebSocketConnectionState::FailedToConnect, UE_WEBSOCKET_CLOSE_APP_FAILURE, FString(TEXT("Failed to create connection")));
		return;
	}
...
void FWinHttpWebSocket::HandleCloseComplete(const EWebSocketConnectionState NewState, const uint16 Code, const FString& Reason)
{
	...
	{
		UE_LOG(LogPassageWebSockets, Warning, TEXT("WinHttp WebSocket[%p]: connection closed before while session creation in progress. Code=[%u] Reason=[%s]"), this, Code, *Reason);
		return;
	}
	...
	switch (PreviousState)
	{
		...
		case EWebSocketConnectionState::Closed:
		{
			...
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("WinHttp WebSocket[%p]: Connection close occurred while in %s state, ignoring. Code=[%u] Reason=[%s]"), this, LexToString(PreviousState), Code, *Reason);
			return;
		}
		case EWebSocketConnectionState::Connecting:
		{
			UE_LOG(LogPassageWebSockets, Log, TEXT("WinHttp WebSocket[%p]: Connection error occurred while connecting. Code=[%u] Reason=[%s]"), this, Code, *Reason);
		...
		case EWebSocketConnectionState::Connected:
		{
			UE_LOG(LogPassageWebSockets, Log, TEXT("WinHttp WebSocket[%p]: Connection closed. Code=[%u] Reason=[%s]"), this, Code, *Reason);
...
{
	UE_LOG(LogPassageWebSockets, Verbose, TEXT("WinHttp WebSocket[%p]: Received connection close event. Code=[%u] Reason=[%s] bWasGraceful=[%d]"), this, Code, *Reason, bGracefulDisconnect);
