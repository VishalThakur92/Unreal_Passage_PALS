...
FPassageLwsWebSocketsManager::FPassageLwsWebSocketsManager()
	: 
#if WITH_SSL
	SslContext(nullptr),
#endif
...
FPassageLwsWebSocketsManager& FPassageLwsWebSocketsManager::Get()
{
	FPassageLwsWebSocketsManager* Manager = static_cast<FPassageLwsWebSocketsManager*>(FPassageWebSocketsModule::Get().WebSocketsManager);
	check(Manager);
	return *Manager;
}

void FPassageLwsWebSocketsManager::InitWebSockets(TArrayView<const FString> Protocols)
{
	...
	for (const FString& Protocol : Protocols)
	{
		...
		LwsProtocol.callback = &FPassageLwsWebSocketsManager::StaticCallbackWrapper;
	...
	{
		UE_LOG(LogPassageWebSockets, Verbose, TEXT("Failed to create our SSL context, this will result in libwebsockets managing its own SSL context, which calls SSLs global cleanup functions, impacting other uses of SSL"));
	}
	...
	{
		UE_LOG(LogPassageWebSockets, Error, TEXT("Failed to initialize libwebsockets"));
		return;
	}
	...
	if (!Thread)
	{
		UE_LOG(LogPassageWebSockets, Error, TEXT("FPassageLwsWebSocketsManager failed to initialize thread!"));
	...
	FTickerDelegate TickDelegate = FTickerDelegate::CreateRaw(this, &FPassageLwsWebSocketsManager::GameThreadTick);
	TickHandle = FBackgroundableTicker::GetCoreTicker().AddTicker(TickDelegate, 0.0f);
}

void FPassageLwsWebSocketsManager::ShutdownWebSockets()
{
...
bool FPassageLwsWebSocketsManager::Init()
{
	return true;
}

uint32 FPassageLwsWebSocketsManager::Run()
{
...
void FPassageLwsWebSocketsManager::Stop()
{
...
void FPassageLwsWebSocketsManager::Exit()
{
	for (FPassageLwsWebSocket* Socket : SocketsTickingOnThread)
	{
...
static void LwsLog(int Level, const char* LogLine)
{
	...
	if (bIsWarning || UE_LOG_ACTIVE(LogPassageWebSockets, Verbose))
	{
		...
		if (bIsWarning)
		{
			UE_LOG(LogPassageWebSockets, Warning, TEXT("Lws(%s): %s"), LwsLogLevelToString(Level), *ConvertedLogLine);
		}
		else
		{
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("Lws(%s): %s"), LwsLogLevelToString(Level), *ConvertedLogLine);
		}
	}
}

int FPassageLwsWebSocketsManager::StaticCallbackWrapper(lws* Connection, lws_callback_reasons Reason, void* UserData, void* Data, size_t Length)
{
	FPassageLwsWebSocketsManager& This = FPassageLwsWebSocketsManager::Get();
	return This.CallbackWrapper(Connection, Reason, UserData, Data, Length);
}

int FPassageLwsWebSocketsManager::CallbackWrapper(lws* Connection, lws_callback_reasons Reason, void* UserData, void* Data, size_t Length)
{
	FPassageLwsWebSocket* Socket = static_cast<FPassageLwsWebSocket*>(UserData);
	...
	case LWS_CALLBACK_OPENSSL_PERFORM_SERVER_CERT_VERIFICATION:
	{
		...
		Socket = static_cast<FPassageLwsWebSocket*>(lws_wsi_user(Connection));
...
void FPassageLwsWebSocketsManager::Tick()
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_FLwsWebSocketsManager_Tick);
	LLM_SCOPE(ELLMTag::Networking);

	{
		FPassageLwsWebSocket* SocketToStart;
	...
	for (FPassageLwsWebSocket* Socket : SocketsDestroyedDuringService)
	{
...
TSharedRef<IWebSocket> FPassageLwsWebSocketsManager::CreateWebSocket(const FString& Url, const TArray<FString>& Protocols, const TMap<FString, FString>& UpgradeHeaders)
{
	...
	FLwsWebSocketRef Socket = MakeShared<FPassageLwsWebSocket>(Url, Protocols, UpgradeHeaderString);
	return Socket;
}

void FPassageLwsWebSocketsManager::StartProcessingWebSocket(FPassageLwsWebSocket* Socket)
{
	Sockets.Emplace(Socket->AsShared());
	SocketsToStart.Enqueue(Socket);
}

bool FPassageLwsWebSocketsManager::GameThreadTick(float DeltaTime)
{
	...
	}
	{
		...
		while (SocketsToStop.Dequeue(Socket))
		{
			...
			TSharedRef<FPassageLwsWebSocket> SocketRef(Socket->AsShared());
