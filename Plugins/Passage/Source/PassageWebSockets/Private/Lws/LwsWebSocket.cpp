...
// FPassageLwsSendBuffer 
FPassageLwsSendBuffer::FPassageLwsSendBuffer(const uint8* Data, SIZE_T Size, bool bInIsBinary)
...
int32 FPassageLwsSendBuffer::GetPayloadSize() const
{
	return Payload.Num() - (int32)(LWS_PRE);
}

// FPassageLwsReceiveBufferBinary
FPassageLwsReceiveBufferBinary::FPassageLwsReceiveBufferBinary(const uint8* Data, const int32 Size, const int32 InBytesRemaining)
...
// FPassageLwsReceiveBufferText
FPassageLwsReceiveBufferText::FPassageLwsReceiveBufferText(FString&& InText)
	: Text(MoveTemp(InText))
{
}

// FPassageLwsWebSocket

int32 FPassageLwsWebSocket::IncrementingIdentifier = 0;

const TCHAR* FPassageLwsWebSocket::ToString(const EState InState)
{
...
FPassageLwsWebSocket::FPassageLwsWebSocket(const FString& InUrl, const TArray<FString>& InProtocols, const FString& InUpgradeHeader)
	: State(EState::None)
...
{
	UE_LOG(LogPassageWebSockets, VeryVerbose, TEXT("FPassageLwsWebSocket[%d]: Constructed url=%s protocols=%s"), Identifier, *InUrl, *FString::Join(Protocols, TEXT(",")));
}

FPassageLwsWebSocket::~FPassageLwsWebSocket()
{
	UE_LOG(LogPassageWebSockets, VeryVerbose, TEXT("FPassageLwsWebSocket[%d]: Destroyed"), Identifier);
	checkf(LwsConnection == nullptr, TEXT("FPassageLwsWebSocket: Must have closed connection before destruction"));
	ClearData();
}

void FPassageLwsWebSocket::Connect()
{
	if (LastGameThreadState != EState::None)
	{
		UE_LOG(LogPassageWebSockets, Warning, TEXT("FPassageLwsWebSocket[%d]::Connect: State is not None (%s), unable to start connecting!"), Identifier, ToString(LastGameThreadState));
		return;
	}
	...
	if (!bDisableDomainWhitelist)
	{
		...
		{
			State = EState::Error;
			UE_LOG(LogPassageWebSockets, Warning, TEXT("FPassageLwsWebSocket[%d]::Connect: %s is not whitelisted. Refusing to connect."), Identifier, *Url);
	...
	{
		UE_LOG(LogPassageWebSockets, Log, TEXT("FPassageLwsWebSocket[%d]::Connect: Domain whitelisting has been disabled by config."), Identifier);
	}
	...
	UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::Connect: setting State=%s url=%s bWantsMessageEvents=%d bWantsRawMessageEvents=%d"), Identifier, ToString(State), *Url, (int32)bWantsMessageEvents, (int32)bWantsRawMessageEvents);

	// Clear up any data from previous runs
	ClearData();

	FPassageLwsWebSocketsManager& WebSocketsManager = FPassageLwsWebSocketsManager::Get();
	WebSocketsManager.StartProcessingWebSocket(this);
}

void FPassageLwsWebSocket::Close(int32 Code, const FString& Reason)
{
	if (CloseRequest.Code != 0)
	{
		UE_LOG(LogPassageWebSockets, Warning, TEXT("FPassageLwsWebSocket[%d]::Close: Already closing, ignoring subsequent attempt"), Identifier);
		return;
	}
	...
	UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::Close: Close queued with code=%d reason=%s"), Identifier, Code, *Reason);
...
void FPassageLwsWebSocket::Send(const void* Data, SIZE_T Size, bool bIsBinary)
{
	SendQueue.Enqueue(new FPassageLwsSendBuffer(static_cast<const uint8*>(Data), Size, bIsBinary));
}

void FPassageLwsWebSocket::Send(const FString& Data)
{
...
void FPassageLwsWebSocket::SendFromQueue()
{
	check(LwsConnection);

	FPassageLwsSendBuffer* CurrentBuffer;
	...
	if (SendQueue.Peek(CurrentBuffer))
	{
		...
		if (!bWriteSuccessful)
		{
			...
			UE_LOG(LogPassageWebSockets, Warning, TEXT("FPassageLwsWebSocket[%d]::SendFromQueue: Error writing buffer Size=%d BytesWritten=%d bIsBinary=%d"), 
		...
		{
			UE_LOG(LogPassageWebSockets, VeryVerbose, TEXT("FPassageLwsWebSocket[%d]::SendFromQueue: Wrote %d bytes, %d bytes remaining in this packet"), 
...
bool FPassageLwsWebSocket::WriteBuffer(FPassageLwsSendBuffer& Buffer)
{
...
int FPassageLwsWebSocket::LwsCallback(lws* Instance, lws_callback_reasons Reason, void* Data, size_t Length)
{
	switch (Reason)
	{
	case LWS_CALLBACK_CLIENT_ESTABLISHED:
	{
		...
		UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLIENT_ESTABLISHED, setting State=%s PreviousState=%s"), Identifier, ToString(State), ToString(PreviousState));
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
	{
		...
		UE_LOG(LogPassageWebSockets, VeryVerbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLIENT_RECEIVE Length=%d BytesLeft=%d"), Identifier, Length, BytesLeft);
		...
		if (bWantsMessageEvents)
		{
            ...
            //if(BytesLeft == 0)
            if(lws_is_final_fragment(Instance))
			{
                // UE_LOG(LogPassageWebSockets, VeryVerbose, TEXT("FPassageLwsWebSocket::LwsCallback: BytesLeft == 0"));
                UE_LOG(LogPassageWebSockets, VeryVerbose, TEXT("FPassageLwsWebSocket::LwsCallback: lws_is_final_fragment"));
				bWakeGameThread = true;
				ReceiveTextQueue.Enqueue(MakeUnique<FPassageLwsReceiveBufferText>(MoveTemp(ReceiveBuffer)));
				ReceiveBuffer.Empty();
			}
		}
		...
		{
			bWakeGameThread = true;
			ReceiveBinaryQueue.Enqueue(MakeUnique<FPassageLwsReceiveBufferBinary>(static_cast<const uint8*>(Data), Length, BytesLeft));
		}
		if (bWakeGameThread)
		{
	...
	case LWS_CALLBACK_WS_PEER_INITIATED_CLOSE:
	{
			...
			State == EState::ClosingByRequest)
		{
			...
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WS_PEER_INITIATED_CLOSE, setting State=%s CloseStatus=%d Reason=%s bPeerSpecifiedReason=%d PreviousState=%s"),
		...
		{
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WS_PEER_INITIATED_CLOSE, but ignoring because our State=%s CloseStatus=%d Reason=%s"),
	...
	case LWS_CALLBACK_WSI_DESTROY:
		...
		if (State == EState::Connecting)
		{
			...
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WSI_DESTROY, setting State=%s PreviousState=%s"), Identifier, ToString(EState::Error), ToString(State));
		...
		else
		{
			...
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_WSI_DESTROY, State=%s"), Identifier, ToString(State));
		}
		LwsConnection = nullptr;
		break;
	case LWS_CALLBACK_CLOSED:
	{
			...
			State == EState::ClosingByRequest)
		{
			...
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLOSED, setting State=%s CloseReason=%s PreviousState=%s"),
		...
		{
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLOSED, ignoring and waiting for LWS_CALLBACK_WSI_DESTROY, State=%s"),
	...
	case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
	{
		...
		bool bWakeGameThread = false;
		UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsCallback: Received LWS_CALLBACK_CLIENT_CONNECTION_ERROR, setting State=%s CloseReason=%s PreviousState=%s"),
	...
	{
		// in FPassageLwsWebSocketsManager::CallbackWrapper, we copied UserData to Data, so Data is the X509_STORE_CTX* instead of the SSL*
...
void FPassageLwsWebSocket::GameThreadTick()
{
	EState CurrentState;
	{
		FScopeLock ScopeLock(&StateLock);
	...
	if (CloseRequest.Code == 0)
	{
		...
		FPassageLwsReceiveBufferBinaryPtr BufferBinary;
...
void FPassageLwsWebSocket::GameThreadFinalize()
{
	...
	UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::GameThreadFinalize: setting State=%s PreviousState=%s"),
...
bool FPassageLwsWebSocket::LwsThreadInitialize(struct lws_context &LwsContext)
{
...
void FPassageLwsWebSocket::LwsThreadTick()
{
	if (State == EState::Connected)
	{
		check(LwsConnection);
		...
		if (State == EState::ClosingByRequest)
		{
			UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::LwsThreadTick: Close requested while connected, setting State=%s PreviousState=%s"),
...
void FPassageLwsWebSocket::ConnectInternal(struct lws_context &LwsContext)
{
	check(!LwsConnection);
	checkf(State == EState::StartConnecting, TEXT("FPassageLwsWebSocket::ConnectInternal: State must be %s, but is %s instead"), ToString(EState::StartConnecting), ToString(State));

	UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::ConnectInternal: setting State=%s PreviousState=%s"),
	...
	if (lws_parse_uri((char*)UrlUtf8.Get(), &UrlProtocol, &ParsedAddress, &ParsedPort, &TmpUrlPath))
	{
		...
		UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::ConnectInternal: setting State=%s PreviousState=%s Reason=%s"),
	...
	{
		UE_LOG(LogPassageWebSockets, Verbose, TEXT("FPassageLwsWebSocket[%d]::ConnectInternal: lws_client_connect_via_info succeeded"), Identifier);
	}
	else
	{
