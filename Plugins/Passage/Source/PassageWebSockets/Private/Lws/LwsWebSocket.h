...
struct FPassageLwsSendBuffer
{
	 ...
	 */
	FPassageLwsSendBuffer(const uint8* Data, const SIZE_T Size, const bool bInIsBinary);
...
struct FPassageLwsReceiveBufferBinary
{
	/**
	 * Constructor
	 * @param Data pointer to data to fill our buffer with
	 ...
	 */
	FPassageLwsReceiveBufferBinary(const uint8* Data, const int32 Size, const int32 InBytesRemaining);
...
typedef TUniquePtr<FPassageLwsReceiveBufferBinary> FPassageLwsReceiveBufferBinaryPtr;

/** Buffer for one incoming text packet, fully received */
struct FPassageLwsReceiveBufferText
{
	/**
	 * Constructor
	 * @param InText The packet contents
	 */
	FPassageLwsReceiveBufferText(FString&& InText);

	/** Text packet received */
	const FString Text;
};

typedef TUniquePtr<FPassageLwsReceiveBufferText> FLwsReceiveBufferTextPtr;

class FPassageLwsWebSocket
	: public IWebSocket
	, public TSharedFromThis<FPassageLwsWebSocket>
{

public:
	/** Destructor */
	virtual ~FPassageLwsWebSocket();
	...
	DECLARE_DERIVED_EVENT(FPassageLwsWebSocket, IWebSocket::FWebSocketConnectedEvent, FWebSocketConnectedEvent);
	...
	DECLARE_DERIVED_EVENT(FPassageLwsWebSocket, IWebSocket::FWebSocketConnectionErrorEvent, FWebSocketConnectionErrorEvent);
	...
	DECLARE_DERIVED_EVENT(FPassageLwsWebSocket, IWebSocket::FWebSocketClosedEvent, FWebSocketClosedEvent);
	...
	DECLARE_DERIVED_EVENT(FPassageLwsWebSocket, IWebSocket::FWebSocketMessageEvent, FWebSocketMessageEvent);
	...
	DECLARE_DERIVED_EVENT(FPassageLwsWebSocket, IWebSocket::FWebSocketRawMessageEvent, FWebSocketRawMessageEvent);
	virtual FWebSocketRawMessageEvent& OnRawMessage() override
	{
		return RawMessageEvent;
	}

	DECLARE_DERIVED_EVENT(FPassageLwsWebSocket, IWebSocket::FWebSocketMessageSentEvent, FWebSocketMessageSentEvent);
...
private:
	/** Constructor */
	FPassageLwsWebSocket(const FString& Url, const TArray<FString>& Protocols, const FString& UpgradeHeader);

	/** Friend for access to constructor via MakeShared */
	friend class SharedPointerInternals::TIntrusiveReferenceController<FPassageLwsWebSocket, ESPMode::ThreadSafe>;
	 ...
	 */
	bool WriteBuffer(FPassageLwsSendBuffer& Buffer);
...
private:
	...
	TQueue<FPassageLwsReceiveBufferBinaryPtr, EQueueMode::Spsc> ReceiveBinaryQueue;
	...
	TQueue<FPassageLwsSendBuffer*, EQueueMode::Spsc> SendQueue;
...
typedef TSharedRef<FPassageLwsWebSocket> FLwsWebSocketRef;
